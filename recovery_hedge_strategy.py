    ...
    try:      
        ...
        with lock:
                ...
                if(changes == True):
                    if((safe_float(positions.data[0].size) > 0 and safe_float(positions.data[1].size) == 0) or (safe_float(positions.data[0].size) == 0 and safe_float(positions.data[1].size) > 0)):      
                        ...
                        if(not recoveryExists):
                            ...
                            if(last_price < recoveryPrice and  recoveryPositionIdx == 1) or (last_price > recoveryPrice and  recoveryPositionIdx == 2): 
                                ...
                                print(f'No se colocó orden recovery, el precio actual es mayor/menor al precio de orden condicional. precio condicional({recoverySide}): {recoveryPrice}, precio actual: {last_price}')
                        if(not tPExists):
            ...
            while not event_queue.empty():
                handle_message(event_queue.get())
...
def connect_to_websocket():
    ...
    while True:
        try:
            ...
            ws.subscribe("position",callback=ws_callback)
